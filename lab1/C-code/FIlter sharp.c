...
int main (int argc, char **argv) 
{
  ...
  for ( i = 0; i < input_img.height; i++ ){
	  ...
	  for (k = -2; k <=2; k++ ){
		  ...
		  color_img.color[0][i][j] = (int)((lamda+1)*img_red[i+2][j+2] - lamda*r_value);
		  color_img.color[1][i][j] = (int)((lamda+1)*img_green[i+2][j+2] - lamda*g_value);
		  color_img.color[2][i][j] = (int)((lamda+1)*img_blue[i+2][j+2] - lamda*b_value);
...
for ( i = 0; i < input_img.height; i++ ){
  for ( j = 0; j < input_img.width; j++ ) {
    if ((int32_t)color_img.color[0][i][j]<0)
          {
            color_img.color[0][i][j]=0;
          }
          else if ((int32_t)color_img.color[0][i][j]>255)
          {
            color_img.color[0][i][j] =255;
          }

          if ((int32_t)color_img.color[1][i][j]<0)
          {
            color_img.color[1][i][j]=0;
          }
          else if ((int32_t)color_img.color[1][i][j]>255)
          {
            color_img.color[1][i][j]=255;
          }

          if ((int32_t)color_img.color[2][i][j]<0)
          {
            color_img.color[2][i][j]=0;
          }
          else if ((int32_t)color_img.color[2][i][j]>255)
          {
  ...
  if ( ( fp = fopen ( "result_Q4_lamda=0.2.tif", "wb" ) ) == NULL ) {
